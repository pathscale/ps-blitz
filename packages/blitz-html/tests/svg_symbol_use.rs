...
#[test]
fn em_sized_production_icon_sprite_paints_visible_strokes() {
    let buffer = render(
        r##"<html><body style="margin:0; background:white;">
            <svg width="0" height="0" style="position:absolute">
            ...
            <svg viewBox="0 0 24 24" width="1em" height="1em"
                 style="font-size:14px; color:rgb(132, 141, 150)"
                 fill="none" stroke="currentColor" stroke-width="2"
                 stroke-linecap="round" stroke-linejoin="round">
                <use href="#i-layout-grid" />
            </svg>
        </body></html>"##,
    );
    ...
    for y in 0..20 {
        for x in 0..20 {
            painted += usize::from(pixel(&buffer, x, y) != [255, 255, 255]);
        }
    }
    assert!(painted > 20, "the production icon must emit visible pixels");
}

#[test]
fn em_sized_icon_paints_inside_a_clipped_flex_button() {
    let buffer = render(
        r##"<html><body style="margin:0; background:white;">
            <svg width="0" height="0" style="position:absolute">
                <symbol id="i-layout-grid" viewBox="0 0 24 24">
            ...
            <div style="display:flex; height:32px; overflow:hidden">
                <button style="display:flex; height:32px; align-items:center; gap:8px; padding-left:14px">
                    <svg viewBox="0 0 24 24" width="1em" height="1em"
                         style="display:block; font-size:14px; color:rgb(132, 141, 150); overflow:hidden"
                         fill="none" stroke="currentColor" stroke-width="2"
                         stroke-linecap="round" stroke-linejoin="round">
                        <use href="#i-layout-grid" />
                    </svg>
    ...
    assert!(painted > 20, "the flex button icon must remain visible");
}
