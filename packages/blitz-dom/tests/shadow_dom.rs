...
use blitz_dom::{BaseDocument, DocumentConfig, LocalName, NodeId, QualName, ShadowRootMode, ns};
...
fn doc_with_body() -> (BaseDocument, NodeId) {
    ...
    // The document root used to be slab index 0; ids are versioned now, so ask
    // the document for it.
    let root = mutator.doc.root_node().id;
