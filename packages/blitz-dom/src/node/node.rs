...
universal_accessors! {
    ...
    // `cache` is not here: it is stored as `Option<Box<Cache>>` and reached
    // through hand-written accessors below, because the read side has to be
    // able to answer without allocating. See `ElementData::cache`.
...
impl Node {
    ...
    /// [`cache`](super::ElementData::cache) field for the measurement behind
    /// that. The signature is unchanged, so callers cannot tell.
    #[inline]
    pub fn cache(&self) -> &Cache {
        match &self.data {
            NodeData::Element(data) | NodeData::AnonymousBlock(data) => data.cache(),
            NodeData::Document(data) => data.cache(),
            _ => panic!("`cache` is not available on this node kind"),
        }
    }
    ...
    pub fn cache_mut(&mut self) -> &mut Cache {
        match &mut self.data {
            NodeData::Element(data) | NodeData::AnonymousBlock(data) => data.cache_mut(),
            NodeData::Document(data) => data.cache_mut(),
            _ => panic!("`cache_mut` is not available on this node kind"),
        }
    }
    ...
    pub fn cache_release(&mut self) {
        match &mut self.data {
            NodeData::Element(data) | NodeData::AnonymousBlock(data) => data.cache_release(),
            NodeData::Document(data) => data.cache_release(),
            _ => {}
        }
    }
